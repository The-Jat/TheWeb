...
class SettingController extends BaseController
{
    ...
    public function getVerifyLicense(Core $coreApi, BaseHttpResponse $response)
    {
        if (!File::exists(storage_path('.license'))) {
            return $response->setError()->setMessage('Nulled By. Nandunkz');
        }
    ...
    public function activateLicense(LicenseSettingRequest $request, BaseHttpResponse $response, Core $coreApi)
    {
        if (filter_var($request->input('buyer'), FILTER_VALIDATE_URL)) {
            ...
            return $response
                ->setError(true)
                ->setMessage('Please try with username "' . $username . '"!');
        }

        try {
            $result = array('status' => true, 'message' => 'Your license is activated successfully!');

            setting()
                ->set(['licensed_to' => 'Nandunkz'])
                ->save();

            $activatedAt = "2022-10-08 00:00:00";

            $data = [
                'activated_at' => Carbon::now()->format('M d Y'),
                'licensed_to'  => 'Nandunkz',
            ];
